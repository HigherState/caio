...
package object arbitrary {
  ...
  implicit def arbitraryForParCaio[C: Arbitrary, L: Arbitrary: Monoid, A: Arbitrary: Cogen]: Arbitrary[ParCaio[C, L, A]] =
    Arbitrary(arbitraryForCaio[C, L, A].arbitrary.map(Par.apply))
  ...
  def genAsyncF[C, L: Monoid, A: Arbitrary]: Gen[Caio[C, L, A]] =
    getArbitrary[(Either[Throwable, A] => Unit) => Unit].map { k =>
      Caio.asyncF[C, L, A](cb => Caio(k(cb)))
    }

  def genTell[C, L: Arbitrary, A: Arbitrary]: Gen[Caio[C, L, A]] =
  ...
  def genNestedAsync[C: Arbitrary, L: Arbitrary: Monoid, A: Arbitrary: Cogen]: Gen[Caio[C, L, A]] =
