...
sealed trait Caio[-C, +V, +L, +A] {
  ...
  def continual[C1 <: C, V1 >: V, L1 >: L, B](f: Either[Throwable, A] => Caio[C1, V1, L1, B])(implicit
    M: Monoid[L1],
    CS: ContextShift[IO]
  ): Caio[C1, V1, L1, B] =
    new CaioConcurrent[C1, V1, L1].continual[A, B](this)(f)
  ...
  def flatTap[C1 <: C, V1 >: V, L1 >: L](f: A => Caio[C1, V1, L1, Any]): Caio[C1, V1, L1, A] =
    flatMap[C1, V1, L1, A](a => f(a).as(a))
  ...
  def rethrow[A1 >: A, B](implicit ev: A1 =:= Either[Throwable, B]): Caio[C, V, L, B] =
