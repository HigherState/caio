...
trait ContextProjector {
  ...
  implicit def askAskProjection[M[_], A, B]
    (implicit AA: InvariantAsk[M, A], M: Mixer[A, B], ev0: A =:!= B, ev1: LowPriority): AskProjection[M, B] =
    AskProjection {
      new MixedAsk[M, A, B](AA)
    }

  implicit def stateStateProjection[M[_], A, B]
    (implicit MS: Stateful[M, A], M: Mixer[A, B], ev: LowPriority): StateProjection[M, B] =
    StateProjection {
