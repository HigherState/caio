...
//    ???
  ...
  implicit def expandsApplicativeMonadStateAskProjection[M[_], M2[_], A, B, C]
    (implicit E:Expands[M2, M, B], A:MonadState[M, A], M:Mixer[(A, B), C], ev:LowPriority):AskProjection[M2, C] = ???


  implicit def expanderExpanderProjection[M[_], A, B]
  (implicit E:Expander[M, A], M:Mixer[A, B], EV: A =:!= B):ExpanderProjection[M, B] =
    ???
